...
namespace LL {
	...
	public:
		...
		//链表版求最大值
		/*INLINED*/Data MY_LIB MaxInAllNodes() const noexcept {
		...
		//链表版求最小值
		/*INLINED*/Data MY_LIB MinInAllNodes()const noexcept {
			const OLL* OprtPtr = this;
		...
		//链表版求平均值
		//链表长度不得超过LONG_MAX
		/*INLINED*/Data MY_LIB Average()const noexcept
		{
			const OLL* OprtPtr = this;
			double average = 0;
		...
		//浅拷贝
		//将清除被赋值对象原有内容
		INLINED OLL& MY_LIB operator=(const OLL& that) noexcept {
			if (&that == this)
			{
				return *this;
			}
