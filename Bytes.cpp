...
template<size_t _Length>class std::numeric_limits<Darkness::LargeInteger::Bytes<_Length>> {
public:
	using b = Darkness::LargeInteger::template Bytes<_Length>;
	static constexpr b max() noexcept { return ~b(0); }
	static constexpr b min()noexcept { return b(0); }
};